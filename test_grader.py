...
def process(scans,ans_key_nums,ans_key_letter,inner,outer,bub_h,bub_w,text_shift,font_size, y1,x1,y2,x2):
	...
	for scan in scans:
		...
		if ans_key_letter:
			image= cv2.putText(image,f"Score = {score} / {len(ans_key_letter)}",(4270-size[0],512),cv2.FONT_HERSHEY_SIMPLEX, 5,(255,255,255),15,cv2.LINE_AA,False)
			image= cv2.putText(image,f"Score = {score} / {len(ans_key_letter)}",(4270-size[0],512),cv2.FONT_HERSHEY_SIMPLEX, 5,(0,0,0),7,cv2.LINE_AA,False)
		image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
